...
Version 0.5: Trust, Alliances, and AI Ladder
...
class Power:
    name: str
    home_provinces: List[str] = field(default_factory=list)
    
    # Existing scores
    conviction_score: int = 5
    attractiveness_score: int = 5
    friction_score: int = 0  # "bureaucratic drag"
    
    # NEW for v0.5
    trust_score: int = 5  # cancels friction 1-for-1
    ai_stage: int = 0  # 0 = none, 1 = Digital Trans, 2 = Predictive, 3 = AI Nexus
    alliances: Set[str] = field(default_factory=set)
    
    color_code: str = ""
    
    # Helpers
    def effective_friction(self) -> int:
        """Friction after trust is taken into account."""
        return max(self.friction_score - self.trust_score, 0)
    
    def ai_bonus(self) -> int:
        """Return cumulative AI bonus for the current ladder stage."""
        return {0: 0, 1: 10, 2: 20, 3: 50}[self.ai_stage]
    
    def to_dict(self):
        return {
            'name': self.name,
            'home_provinces': self.home_provinces,
            'ai_stage': self.ai_stage,
            'scores': {
                'conviction': self.conviction_score,
                'attractiveness': self.attractiveness_score,
                'friction': self.friction_score,
                'trust': self.trust_score,
                'effective_friction': self.effective_friction()
            },
...
class ConvictionGame:
    ...
    def create_simple_map(self):
        ...
        # Create two powers
        red = Power("Red", ["North", "West"], color_code="R")
        blue = Power("Blue", ["East", "South"], color_code="B")
    ...
    def add_alliance(self, p1: str, p2: str) -> None:
        ...
        self.powers[p1].attractiveness_score = min(10, self.powers[p1].attractiveness_score + 1)
        self.powers[p2].attractiveness_score = min(10, self.powers[p2].attractiveness_score + 1)
        
        self.last_results.append(
            f"Alliance formed: {p1} ↔ {p2} (+1 Attractiveness each)"
        )
    ...
    def break_alliance(self, p1: str, p2: str) -> None:
        ...
        if p2 in self.powers[p1].alliances:
            ...
            # Betrayal penalty
            self.powers[p1].conviction_score = max(0, self.powers[p1].conviction_score - 2)
            self.powers[p2].conviction_score = max(0, self.powers[p2].conviction_score - 1)
            
            self.last_results.append(
                f"Alliance broken: {p1} X {p2} ({p1} -2 Conv, {p2} -1 Conv)"
            )
    ...
    def calculate_power_projection(self, power_name: str) -> int:
        ...
        # Territory & military
        supply_centers = sum(
            1 for p in self.provinces.values()
        ...
        military = units * 10
        conviction_bonus = power.conviction_score * 2
        attractiveness = power.attractiveness_score * 3
        ai_bonus = power.ai_bonus()
        
        # NEW: network bonus = 2 pts per alliance edge
        network_bonus = len(power.alliances) * 2
        
        total = (
            territorial + military + conviction_bonus +
            attractiveness + ai_bonus + network_bonus
        )
    ...
    def draw_power_projection_bar(self):
        ...
        print(f"[{bar}]")
        print(f"Red: {red_power:3.0f} pts {'*WIN*' if red_power >= self.victory_threshold else ''}")
        print(f"Blue: {blue_power:3.0f} pts {'*WIN*' if blue_power >= self.victory_threshold else ''}")
    ...
    def show_power_details(self):
        ...
        for power_name in ["Red", "Blue"]:
            ...
            ai_stage_name = self.AI_LADDER[power.ai_stage - 1] if power.ai_stage > 0 else "None"
            
            print(f"\n{power_name} (AI Stage: {ai_stage_name}):")
            print(f"  Territorial Control: {proj.get('territorial', 0):3.0f} pts")
            print(f"  Military Strength:   {proj.get('military', 0):3.0f} pts")
            print(f"  Conviction Bonus:    {proj.get('conviction', 0):3.0f} pts (Conv: {power.conviction_score})")
            print(f"  Attractiveness:      {proj.get('attractiveness', 0):3.0f} pts (Attr: {power.attractiveness_score})")
            print(f"  AI Stage Bonus:      {proj.get('ai', 0):3.0f} pts (Stage {power.ai_stage}/3)")
            print(f"  Network Bonus:       {proj.get('network', 0):3.0f} pts ({len(power.alliances)} allies)")
            print(f"  ---")
            print(f"  Trust Score:         {power.trust_score}")
            print(f"  Friction:            {power.friction_score}")
            print(f"  Effective Friction:  {power.effective_friction()}")
            print(f"  ---")
    ...
    def get_human_orders(self, power_name: str) -> List[Order]:
        ...
        power = self.powers[power_name]
        if power.ai_stage < 3:
            next_stage = self.AI_LADDER[power.ai_stage]
            threshold = 4 + power.effective_friction() + power.ai_stage
            
            print(f"\nAI Integration Status:")
            print(f"  Current Stage: {power.ai_stage}/3")
    ...
    def _resolve_ai_orders(self, ai_orders: List[Order]) -> None:
        ...
        for order in ai_orders:
            ...
            next_stage_name = self.AI_LADDER[power.ai_stage]
            threshold = 4 + power.effective_friction() + power.ai_stage
            roll = random.randint(1, 10)
            
            if roll >= threshold:
                power.ai_stage += 1
                power.friction_score += 1  # organizational growing pains
                
                # Stage-specific bonuses
                if power.ai_stage == 1:
                    power.attractiveness_score = min(10, power.attractiveness_score + 1)
                elif power.ai_stage == 2:
                    power.trust_score = min(10, power.trust_score + 1)
                elif power.ai_stage == 3:
                    power.conviction_score = min(10, power.conviction_score + 1)
                ...
                power.friction_score += 1  # failed attempt still hurts
                self.last_results.append(
                    f"{order.power} failed {next_stage_name} "
                    f"(roll {roll} < {threshold}); Friction +1"
                )
    ...
    def resolve_orders(self, all_orders: List[Order]):
        ...
        for destination, orders in destinations.items():
            if len(orders) == 1:
                ...
                self.last_results.append(f"{order.power} Army: {old_location} -> {destination}")
            else:
                for order in orders:
                    unit = self.units[order.unit_id]
                    self.last_results.append(f"{order.power} Army bounced trying to enter {destination}")
    ...
    def process_random_events(self):
        ...
        for power_name, power in self.powers.items():
            # Low conviction might cause desertion
            if power.conviction_score <= 2 and random.random() < 0.2:
                self.last_results.append(f"{power_name}: Low conviction causing unrest!")
                power.attractiveness_score = max(0, power.attractiveness_score - 1)
            
            # High attractiveness might provide benefits
            if power.attractiveness_score >= 8 and random.random() < 0.3:
                self.last_results.append(f"{power_name}: High attractiveness attracts support!")
                power.conviction_score = min(10, power.conviction_score + 1)
            
            # Low trust creates problems
            if power.trust_score <= 2 and random.random() < 0.3:
                self.last_results.append(f"{power_name}: Low trust causing institutional decay!")
                power.friction_score += 1
    ...
    def play_turn(self, bot_strategies=None):
        ...
        if winner:
            ...
            print(f"Final Power Projection: {self.calculate_power_projection(winner):.0f}")
            print(f"{'*'*50}")
            return False
...
def random_bot_strategy(game_state: dict) -> list:
    ...
    if power["ai_stage"] < 3 and random.random() < 0.3:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Conviction v0.5: The AI Integration Strategy Game"
    )
    parser.add_argument(
        "--bot", 
        nargs=2, 
    ...
    print("CONVICTION v0.5: The AI Integration Strategy Game")
    print("="*50)
    print("NEW FEATURES:")
    print("  - Trust Score: Reduces effective friction")
    print("  - Alliance Networks: +2 points per alliance")
    print("  - AI Ladder: 3 stages of technological advancement")
    print("="*50)
    print("\nGoal: Reach 100 Power Projection points")
    print("\nPower comes from:")
    print("  - Territory (15 pts per supply center)")
    print("  - Military (10 pts per unit)")
    print("  - Conviction (2 pts per point)")
    print("  - Attractiveness (3 pts per point)")
    print("  - AI Stages (10/20/50 pts cumulative)")
